...
def test_forward_pass(model, model_name):
    ...
    # Create dummy input (batch_size=2, channels=1, height=8, width=131072)
    batch_size = 2
    input_tensor = torch.randn(batch_size, 1, 8, 131072)
...
def test_model_parameters(model, model_name):
    ...
    # Test gradient flow
    input_tensor = torch.randn(1, 1, 8, 131072, requires_grad=True)
    reconstructed, latent = model(input_tensor)
...
def test_model_save_load(model, model_name):
    ...
    try:
        ...
        # Test loaded model
        input_tensor = torch.randn(1, 1, 8, 131072)
